<x-app-layout>
    <div class="flex-1 p-6">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-2xl font-bold text-gray-800 dark:text-gray-100">Data Pestisida</h2>
        </div>
        <div class="bg-white dark:bg-gray-800 shadow-md rounded-lg p-6">
            <div class="mb-4 flex justify-between items-center">
                <div class="relative">
                    <input type="text" id="search" placeholder="Cari Pestisida..." class="border border-gray-300 rounded-md px-4 py-2 w-64 focus:outline-none focus:ring-2 focus:ring-blue-500" onkeyup="filterTable()">
                    <script>
                        function filterTable() {
                            ...
                            for (let i = 1; i < rows.length; i++) { // Start from 1 to skip header
                                ...
                                const namaPestisida = cells[1].textContent.toLowerCase();

                                if (namaPestisida.includes(filter)) {
                            ...
                            if (!found && filter.length > 0) { // Check if filter is not empty
                                if (!notification) {
                                    ...
                                    noResults.textContent = 'Nama Pestisida yang dicari tidak ada.';
                            ...
                            <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                                <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
                                    <h3 class="text-lg leading-6 font-medium text-gray-900" id="modal-title">
                                        Tambah Data Pestisida
                                    </h3>
                                </div>
                                <div class="mt-2">
                                    <form class="space-y-4" action="{{ route('data-pestisida.store') }}" method="post" enctype="multipart/form-data">
                                        @csrf
                                        <div>
                                            <label for="name" class="block text-sm font-medium text-gray-700">Nama Pestisida</label>
                    ...
                    <tbody class="bg-white dark:bg-gray-800">
                        ...
                        <tr>
                            <td colspan="4" class="px-6 py-4 text-center text-gray-500 dark:text-gray-400">
                                <p class="text-lg font-semibold">Data pestisida kosong</p>
                                <p class="mt-1">Belum ada pestisida yang terdaftar.</p>
                            </td>
                        </tr>
                        @else
                        @foreach ($pestisida as $index => $pestisida)
                        <tr>
                            <td class="px-6 py-4 text-sm font-medium text-gray-900 dark:text-gray-100 text-center">{{ $index + 1 }}</td>
                            <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">{{ $pestisida->name }}</td>
                            <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400 flex items-center justify-center">
                                <img src="{{ asset('storage/' . $pestisida->image) }}" alt="{{ $pestisida->name }}" class="w-40 h-40 object-cover">
                            </td>
                            <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400 text-center">
                                <button type="button" onclick="openDeleteModal('{{ $pestisida->id }}')" class="inline-flex items-center px-4 py-2 bg-red-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-red-700 active:bg-red-900 focus:outline-none focus:border-red-900 focus:ring ring-red-300 disabled:opacity-25 transition ease-in-out duration-150">
                                    Hapus
                                </button>
                                <div id="delete-modal-{{ $pestisida->id }}" class="fixed z-10 inset-0 hidden" aria-labelledby="modal-title" role="dialog" aria-modal="true">
                                    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                                        <div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" aria-hidden="true"></div>
                                        <span class="hidden sm:inline-block sm:align-middle sm:h-screen" aria-hidden="true">&#8203;</span>
                                        <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                                                    ...
                                                    <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                                        ...
                                                        <div class="mt-2">
                                                            <p class="text-sm text-gray-500">
                                                                Apakah Anda yakin ingin menghapus data "{{ $pestisida->name }}"? Data yang dihapus tidak dapat dikembalikan.
                                                            </p>
                                                        </div>
                                                    </div>
                                                </div>
                                            ...
                                            <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                                                <form action="{{ route('data-pestisida.destroy', $pestisida->id) }}" method="POST">
                                                    @csrf
                                                    @method('DELETE')
